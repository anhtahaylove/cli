...
3. **回读抽样校验**：写完关键值 / 普通公式后，用 `+csv-get` 或 `+cells-get` 重新读取写入区域，至少抽样 3-5 个代表性单元格（首 / 中 / 末），核对值与预期一致（与本地脚本计算的预期值对照）。AI 公式的计算状态不要先用 `+cells-get` 轮询，直接按 `lark-sheets-formula-verify` 的 `+formula-verify --ai-only --range` 异步抽检规则处理。公式特定的"先验证模板再 --copy-to-range / 修完再读回"细则见下方相关章节。
...
1. **写完建议读回**：`+cells-set` 后紧跟 `+csv-get`（或 `+cells-get`）读目标范围前几行，检查是否出现 `#VALUE!` / `#NAME?` / `#REF!` / `#N/A` / `#DIV/0!` / `#NUM!`；AI 公式在这一步只做**一次**公式文本核对（`+cells-get --include formula` 看种子格 / 首格，确认引号 / 括号没被 shell / CSV / JSON 层破坏、落进去的确实是 `=AI(...)`），不要用它轮询计算结果——计算状态走 `+formula-verify --ai-only --range`（详见 `lark-sheets-formula-verify`）
...
💡 **公式写入后的补充诊断可用 `+formula-verify`**：`+csv-get` / `+cells-get` 的抽样回读只能帮你快速发现明显错误，但它覆盖不到整列中段、隐藏行、被条件格式遮蔽的错误，也看不到 `partial` 截断。如果这次 `+cells-set` / `--copy-to-range` / `+csv-put` 实际写入了公式，可转到 `lark-sheets-formula-verify` 跑 `+formula-verify` 做诊断。普通公式跑到 `status='success'`；AI 公式先对种子格 / 首格做一次 `+cells-get --include formula` 核对文本，再用 `+formula-verify --ai-only --range <整个写入区间>` 校验状态（判据 `ai_formula_failed_count == 0` 且 `ai_formula_total == 预期条数`），满足后即使仍有 pending 也可交付并告知用户后台仍在计算。不要等用户补一句"再验证下公式"才想到这个工具。
