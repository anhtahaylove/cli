...
Table 类型另有专属命令：`+table-update --table-id <id|name> --name <new>` 改表名，`+table-delete --table-id <id|name> --yes` 删表。两者的 `--table-id` 都接受表 ID 或当前 Base 内的表名；传表名时该值会直接进入请求路径，删除前务必确认目标唯一。`--yes` 的 confirmation gate 只在真实执行时生效，`--dry-run` 不会因为缺少 `--yes` 而拒绝，所以不能用 dry-run 来验证自己是否漏掉了确认。
