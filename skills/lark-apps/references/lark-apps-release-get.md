...
- `status=failed` / `rejected` / `canceled` 都是未部署成功的终态：`rejected` 表示发布审批被拒绝，`canceled` 表示发布被取消。若输出含 `error_logs`（`step`/`error_log`），据此向用户转述关键失败步骤或审批结果；未返回时不要编造原因。
...
8. **终态**：`finished` 按 `online_url` 的可选输出规则报告；`failed` / `rejected` / `canceled` 按 `error_logs` 的可选输出规则报告，并明确本轮没有部署成功。`current_node_info.current_status` 只有 `PENDING` 会改变轮询策略，其他值不用于推断发布结果。
9. **未知状态**：`status` 不是 `publishing`、`finished`、`failed`、`rejected` 或 `canceled` 时，停止自动轮询，原样报告 `release_id` 和 status；不要自行判定成功或失败，也不要新建 release 代替查询。
